...
# Actual data: http://py4e-data.dr-chuck.net/comments_913542.xml (Sum ends with 90)
# You do not need to save these files to your folder since your program will read
#  the data directly from the URL. Note: Each student will have a distinct
...
file_xml = urllib.request.urlopen(location, context=ctx).read()

words = 0
characters = 0
for lineno, line in enumerate(file_xml, 1):
    if (type(line) == int):
        line = str(line)
    wordslist = line.split()
    words += len(wordslist)

print("Retrieved ",words," characters")
...
for numbs in numbers:
    print(numbs.find('name').text)
    i_num = int(numbs.find('count').text)
    total_sum += i_num
